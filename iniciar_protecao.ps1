...
if ($PSScriptRoot) {
    ...
    # O projeto está no mesmo diretório do script (sistema_exodo_01-12)
...
if (Test-Path $expectedPath) {
    ...
    Write-Host "Usando caminho do projeto: $projectPath" -ForegroundColor Gray
} else {
    Write-Host "AVISO: Caminho esperado não encontrado: $expectedPath" -ForegroundColor Yellow
    Write-Host "Usando caminho detectado: $projectPath" -ForegroundColor Yellow
}
...
if ($gitCheck) {
    # Garantir que estamos no diretório raiz do Git
    ...
    if ($status) {
        ...
        Write-Host "Diretório do commit: $(Get-Location)" -ForegroundColor Gray
