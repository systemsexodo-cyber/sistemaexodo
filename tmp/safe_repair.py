...
# 1. Renomear import para permitir subclassificacao
print("Renomeando import de ComunicacaoSefaz...")
text = text.replace('from pynfe.processamento.comunicacao import ComunicacaoSefaz', 
                    'from pynfe.processamento.comunicacao import ComunicacaoSefaz as OriginalComunicacaoSefaz')

# 2. Inserir a classe ComunicacaoSefaz (subclass) logo apos os imports
if 'class ComunicacaoSefaz' not in text:
    print("Inserindo classe ComunicacaoSefaz...")
    insertion_point = text.find('from pynfe.processamento.comunicacao import OriginalComunicacaoSefaz')
...
"""
        text = text[:end_line] + subclass_code + text[end_line:]

# 3. Substituir a funcao cancelar_nfce_pynfe
...
def cancelar_nfce_pynfe(req_dict):
    ...
    try:
        ...
        try:
            # Usar a nossa classe ComunicacaoSefaz customizada
