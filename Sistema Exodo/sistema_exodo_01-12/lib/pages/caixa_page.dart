...
class _CaixaPageState extends State<CaixaPage> {
  ...
  void _mostrarDialogoFechamento(BuildContext context, DataService dataService) {
    ...
    // Calcular sangrias (se disponível no DataService)
    double totalSangrias = 0.0;
    try {
      // Tentar acessar sangrias se existir no DataService
      if (dataService.fechamentosCaixa.isNotEmpty) {
        final fechamentoAtual = dataService.fechamentosCaixa
            .where((f) => f.aberturaCaixaId == abertura.id)
            .firstOrNull;
        if (fechamentoAtual != null) {
          totalSangrias = fechamentoAtual.totalSangrias;
        }
      }
    } catch (e) {
      // Se não houver sangrias, usar 0
      totalSangrias = 0.0;
    }
    
    final valorEsperadoCalculado = abertura.valorInicial + totalVendas - totalSangrias;
