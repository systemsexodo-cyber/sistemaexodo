...
class _EntregaDetalhesPageState extends State<EntregaDetalhesPage> {
  ...
  void initState() {
    ...
    // Adicionar listener para buscar taxa automaticamente quando bairro for preenchido
    _bairroController.addListener(_buscarTaxaAutomaticamente);
    _cidadeController.addListener(_buscarTaxaAutomaticamente);
    _cepController = TextEditingController(text: e?.cep ?? '');
    
    // Se já houver bairro preenchido e não houver taxa já definida, buscar taxa automaticamente
    if (e?.bairro != null && e!.bairro!.isNotEmpty && e.taxaEntrega == null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          _buscarTaxaAutomaticamente();
        }
      });
    }
    ...
    if (e?.taxaEntrega != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        final dataService = Provider.of<DataService>(context, listen: false);
        try {
          // Tentar encontrar taxa pelo valor
          final taxaEncontrada = dataService.taxasEntrega.firstWhere(
            (t) => t.ativo && t.valor == e!.taxaEntrega,
          );
          if (mounted) {
            setState(() {
              _taxaEntregaId = taxaEncontrada.id;
              _usarTaxaManual = false;
            });
          }
        } catch (_) {
          // Taxa não encontrada - usar valor manual
          if (mounted) {
            setState(() {
              _usarTaxaManual = true;
              _taxaEntregaId = null;
            });
          }
        }
    ...
    // Inicializar taxa de entrega se existir
    if (e?.taxaEntrega != null) {
      // Buscar a taxa pelo valor e bairro/cidade para encontrar o ID correto
      WidgetsBinding.instance.addPostFrameCallback((_) {
        final dataService = Provider.of<DataService>(context, listen: false);
        try {
          // Tentar encontrar taxa pelo valor e bairro/cidade
          final taxaEncontrada = dataService.taxasEntrega.firstWhere(
            (t) => t.ativo && 
                   t.valor == e!.taxaEntrega &&
                   (e.bairro == null || t.bairro.toLowerCase() == e.bairro!.toLowerCase()) &&
                   (e.cidade == null || t.cidade == null || t.cidade!.toLowerCase() == e.cidade!.toLowerCase()),
            orElse: () => dataService.taxasEntrega.firstWhere(
              (t) => t.ativo && t.valor == e!.taxaEntrega,
            ),
          );
          if (mounted) {
          ...
          // Taxa não encontrada ou múltiplas taxas com mesmo valor
          // Tentar encontrar apenas pelo valor
          try {
            final taxaPorValor = dataService.taxasEntrega.firstWhere(
              (t) => t.ativo && t.valor == e!.taxaEntrega,
            );
            if (mounted) {
              setState(() {
                _taxaEntregaId = taxaPorValor.id;
              });
            }
          } catch (_) {
            // Taxa não encontrada - deixar vazio
            debugPrint('Taxa de entrega não encontrada para valor: ${e!.taxaEntrega}');
          }
        }
      });
    }
  ...
  @override
  void dispose() {
    _bairroController.removeListener(_buscarTaxaAutomaticamente);
    _cidadeController.removeListener(_buscarTaxaAutomaticamente);
  ...
  Widget build(BuildContext context) {
    try {
      ...
      debugPrint('Erro ao construir página de detalhes da entrega: $e');
      return AppTheme.appBackground(
        child: Scaffold(
          backgroundColor: Colors.transparent,
          appBar: AppBar(
  ...
  Widget _buildStatusSection(DataService dataService) {
    if (widget.entrega == null) return const SizedBox.shrink();
  ...
  Widget _buildTaxaEntregaSelector(DataService dataService) {
    try {
      final taxas = dataService.taxasEntrega.where((t) => t.ativo).toList();
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                ...
                if (_bairroController.text.isNotEmpty)
                  TextButton(
                    onPressed: () => _buscarTaxaPorBairro(
            ...
            const SizedBox(height: 12),
            // Toggle entre taxa cadastrada e manual
            Row(
              children: [
                Expanded(
                  child: ChoiceChip(
                    label: const Text('Taxa Cadastrada'),
                    selected: !_usarTaxaManual,
                    onSelected: (selected) {
                      setState(() {
                        _usarTaxaManual = !selected;
                        if (selected) {
                          _taxaManualController.clear();
                        } else {
                          _taxaEntregaId = null;
                        }
                      });
                    ...
                    selectedColor: Colors.orange,
                    labelStyle: TextStyle(
                      color: _usarTaxaManual ? Colors.white70 : Colors.white,
                      fontWeight: _usarTaxaManual ? FontWeight.normal : FontWeight.bold,
                    ),
                  ),
                ),
                const SizedBox(width: 8),
                Expanded(
                  child: ChoiceChip(
                    ...
                    selectedColor: Colors.orange,
                    labelStyle: TextStyle(
                      color: _usarTaxaManual ? Colors.white : Colors.white70,
                      fontWeight: _usarTaxaManual ? FontWeight.bold : FontWeight.normal,
                    ),
                  ),
                ),
              ],
            ...
            const SizedBox(height: 12),
            if (!_usarTaxaManual) ...[
              // Seletor de taxa cadastrada
              if (taxas.isNotEmpty)
                DropdownButtonFormField<String>(
                  value: _taxaEntregaId,
                  isExpanded: true,
                  decoration: InputDecoration(
                    labelText: 'Selecionar Taxa',
                  ...
                  style: const TextStyle(color: Colors.white),
                  items: [
                    const DropdownMenuItem(value: null, child: Text('Sem taxa')),
                    ...taxas.map(
                      (t) => DropdownMenuItem(
                        value: t.id,
                        child: Text(
                          '${t.bairro}${t.cidade != null ? ' - ${t.cidade}' : ''} - R\$ ${t.valor.toStringAsFixed(2)}',
                ...
                Container(
                  ...
                  child: const Text(
                    'Nenhuma taxa cadastrada. Use valor manual.',
                    style: TextStyle(color: Colors.orange, fontSize: 12),
                  ),
                ),
              if (_taxaEntregaId != null) ...[
                const SizedBox(height: 8),
                Builder(
                  builder: (context) {
                    try {
                      final taxaSelecionada = taxas.firstWhere((t) => t.id == _taxaEntregaId);
                      return Container(
                        padding: const EdgeInsets.all(8),
                        decoration: BoxDecoration(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Flexible(
                              child: Text(
                                'Taxa: R\$ ${taxaSelecionada.valor.toStringAsFixed(2)}',
                                style: const TextStyle(
              ...
              if (_taxaManualController.text.isNotEmpty) ...[
                const SizedBox(height: 8),
                Container(
                  padding: const EdgeInsets.all(8),
                  decoration: BoxDecoration(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Flexible(
                        child: Text(
                          'Taxa Manual: R\$ ${_taxaManualController.text}',
                          style: const TextStyle(
      ...
      debugPrint('Erro ao construir seletor de taxa: $e');
      return const SizedBox.shrink();
    }
  }

  void _buscarTaxaAutomaticamente() {
    final bairro = _bairroController.text.trim();
    if (bairro.isEmpty) return;
    
    // Só buscar se não estiver usando taxa manual e não tiver taxa já selecionada
    if (_usarTaxaManual || _taxaEntregaId != null) return;

    final dataService = Provider.of<DataService>(context, listen: false);
    final cidade = _cidadeController.text.trim();
    
    final taxa = dataService.getTaxaEntregaPorBairro(
      bairro,
      cidade: cidade.isEmpty ? null : cidade,
    );

    if (taxa != null && mounted) {
      setState(() {
        _taxaEntregaId = taxa.id;
        _usarTaxaManual = false;
      });
      debugPrint('Taxa encontrada automaticamente para bairro "$bairro": R\$ ${taxa.valor.toStringAsFixed(2)}');
    } else {
      debugPrint('Nenhuma taxa encontrada para bairro "$bairro"');
    }
  }

  void _buscarTaxaPorBairro(String bairro, String cidade) {
    if (bairro.isEmpty) return;
    ...
    if (taxa != null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Taxa encontrada: R\$ ${taxa.valor.toStringAsFixed(2)}',
            ),
            backgroundColor: Colors.green,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Nenhuma taxa encontrada para este bairro'),
          backgroundColor: Colors.orange,
  ...
  void _salvar(DataService dataService) {
    ...
    final entrega = Entrega(
      ...
      taxaEntrega: _usarTaxaManual
          ? (double.tryParse(_taxaManualController.text.replaceAll(',', '.')) ?? null)
          : (_taxaEntregaId != null
              ? () {
                  try {
                    final taxa = dataService.taxasEntrega.firstWhere(
                      (t) => t.id == _taxaEntregaId,
                    );
                    return taxa.valor;
                  } catch (_) {
                    return null;
    ...
    // Se houver taxa de entrega e a entrega estiver vinculada a um pedido, somar ao pedido
    if (_taxaEntregaId != null && entrega.pedidoId.isNotEmpty) {
      try {
        final taxa = dataService.taxasEntrega.firstWhere(
          (t) => t.id == _taxaEntregaId,
        );
        final pedidoIndex = dataService.pedidos.indexWhere(
          (p) => p.id == entrega.pedidoId,
        );
        
        if (pedidoIndex != -1) {
          final pedido = dataService.pedidos[pedidoIndex];
          
          // Verificar se a taxa já foi adicionada ao pedido (evitar duplicação)
          final taxaJaAdicionada = pedido.servicos.any(
            (s) => s.descricao.toLowerCase().contains('taxa de entrega'),
          );
          
          if (!taxaJaAdicionada) {
            // Adicionar taxa de entrega como um serviço no pedido
            final servicoTaxa = ItemServico(
              id: DateTime.now().millisecondsSinceEpoch.toString(),
              descricao: 'Taxa de Entrega - ${_bairroController.text.isNotEmpty ? _bairroController.text : "Entrega"}',
              valor: taxa.valor,
              valorAdicional: 0.0,
              dataAgendamento: DateTime.now(),
            );
            
            final servicosAtualizados = [...pedido.servicos, servicoTaxa];
            final observacoesComTaxa = pedido.observacoes != null && pedido.observacoes!.isNotEmpty
                ? '${pedido.observacoes}\nTaxa de entrega: R\$ ${taxa.valor.toStringAsFixed(2)}'
                : 'Taxa de entrega: R\$ ${taxa.valor.toStringAsFixed(2)}';
            
            final pedidoAtualizado = pedido.copyWith(
              servicos: servicosAtualizados,
              observacoes: observacoesComTaxa,
        ...
        // Pedido não encontrado ou taxa não encontrada - continuar normalmente
        debugPrint('Erro ao adicionar taxa ao pedido: $e');
      }
    }
