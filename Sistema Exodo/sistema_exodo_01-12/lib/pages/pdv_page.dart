...
class _PdvPageState extends State<PdvPage> with SingleTickerProviderStateMixin {
        ...
        dataService.pedidos
            .where(
              (p) {
                ...
                // Incluir vendas salvas (tipo "outro" sem recebimento)
                final nenhumRecebido = p.pagamentos.every((pag) => !pag.recebido);
                final temFiado = p.pagamentos.any((pag) => pag.tipo == TipoPagamento.fiado);
                final tipoPagamento = p.pagamentos.isNotEmpty ? p.pagamentos.first.tipo : null;
                final isVendaSalva = nenhumRecebido && 
                    p.status == 'Pendente' && 
                    !temFiado &&
                    p.pagamentos.isNotEmpty &&
  ...
  Widget _buildItemPedidoPendente(Pedido pedido, DataService dataService) {
    ...
    // Verificar se é venda salva (nenhum pagamento recebido E não é fiado E tipo "outro")
    ...
    // Venda salva: nenhum recebido, status Pendente, não é fiado, e tipo é "outro"
    // Não considerar pedidos sem pagamentos como "venda salva" - são pedidos normais
    final isVendaSalva = nenhumRecebido && 
                  ...
                  child: Row(
                    children: [
                      ...
                      if (!isCancelado) ...[
                        ...
                        Builder(
                          builder: (context) {
                            final nenhumRecebido = pedido.pagamentos.every(
                              (p) => !p.recebido,
                            );
                            ...
                            // Venda salva: nenhum recebido, status Pendente, não é fiado, tipo "outro"
                            // Não considerar pedidos sem pagamentos como venda salva
                            final tipoPag = pedido.pagamentos.isNotEmpty 
                                ? pedido.pagamentos.first.tipo 
                                : null;
                            final isVendaSalvaLocal =
                                nenhumRecebido &&
                                pedido.status == 'Pendente' &&
                                !temFiadoPendente &&
                                pedido.pagamentos.isNotEmpty &&
