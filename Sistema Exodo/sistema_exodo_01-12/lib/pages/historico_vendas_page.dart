...
class _HistoricoVendasPageState extends State<HistoricoVendasPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final pedidosComRecebimento = dataService.pedidos.where((p) {
      // Verificar se é venda salva (tipo "outro")
      final isVendaSalva = p.pagamentos.isNotEmpty &&
      ...
      final temRecebimentoNoPeriodo = p.pagamentos.any((pag) {
        // Só considerar pagamentos recebidos com data de recebimento
      ...
      // Se é venda salva, só incluir se tiver recebimento no período
      // Se não é venda salva, incluir se tiver recebimento no período
      if (isVendaSalva) {
        return temRecebimentoNoPeriodo; // Vendas salvas só aparecem quando recebidas
      }
    ...
    for (final venda in vendasBalcao) {
      // Excluir vendas salvas que ainda não foram recebidas
      // Venda salva é identificada pelo tipoPagamento ser "outro"
      // Só aparece no histórico se tiver valorRecebido (foi recebida)
      if (venda.tipoPagamento == TipoPagamento.outro) {
        // Se é venda salva, só incluir se foi recebida (tem valorRecebido)
        if (venda.valorRecebido == null || venda.valorRecebido == 0) {
          continue; // Pular vendas salvas não recebidas
        }
      }
  ...
  void _mostrarResumoCaixas(BuildContext context, List<ItemHistorico> itensHistorico) {
    try {
      ...
      for (final venda in vendasBalcaoDoCaixa) {
        ...
        // Para vendas parceladas no crediário ou boleto, só contabiliza quando receber
        final tiposParcelaveis = [TipoPagamento.crediario, TipoPagamento.boleto];
        double valorContabilizado;
