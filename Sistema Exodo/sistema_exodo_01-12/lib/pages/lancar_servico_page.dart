...
class _LancarServicoPageState extends State<LancarServicoPage> {
  ...
  void _adicionarServico() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          valorAdicional > 0
              ? 'Serviço adicionado! Base: R\$ ${precoBase.toStringAsFixed(2)} + Adicional: R\$ ${valorAdicional.toStringAsFixed(2)} = Total: R\$ ${totalServico.toStringAsFixed(2)}'
              : 'Serviço adicionado! Valor: R\$ ${precoBase.toStringAsFixed(2)}',
        ),
        ...
        children: [
          ...
          Autocomplete<Servico>(
            ...
            onSelected: (Servico servico) async {
              ...
              // Se tiver valor adicional, auto-adicionar o serviço sem preencher campos
              if (servico.temAdicional) {
                // Marcar que está adicionando
                _adicionandoServico = true;
                
                try {
                  // Mostrar diálogo para selecionar funcionário e comissão
                  final resultado = await _mostrarDialogoFuncionarioComissao(context);
                  if (resultado != null) {
                    final funcionario = resultado['funcionario'] as Funcionario?;
                    final comissao = resultado['comissao'] as double;
                    
                    // Adicionar serviço automaticamente
                    await _adicionarServicoAutomatico(servico, funcionario, comissao);
                    
                    // Atualizar controle de duplicação
                    _ultimoServicoAdicionadoId = servico.id;
                    _ultimaAdicaoServico = agora;
                    
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          'Serviço "${servico.nome}" adicionado automaticamente! Base: R\$ ${valorBase.toStringAsFixed(2)} + Adicional: R\$ ${valorAdicional.toStringAsFixed(2)} = Total: R\$ ${servico.precoTotal.toStringAsFixed(2)}',
                        ),
                        backgroundColor: Colors.green,
                        duration: const Duration(seconds: 2),
                      ),
                ...
                // Preencher os campos do formulário apenas quando não for auto-adicionar
                setState(() {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                      'Serviço "${servico.nome}" carregado! Preencha os campos e clique em "Adicionar Serviço"',
                    ),
                    backgroundColor: Colors.blue,
                    duration: const Duration(seconds: 3),
                  ),
                      ...
                      child: DropdownButtonFormField<Funcionario?>(
                        ...
                        items: [
                          const DropdownMenuItem<Funcionario?>(
                            value: null,
                            child: Text('Selecione o funcionário', style: TextStyle(color: Colors.white70)),
                          ),
              ...
              children: [
                ...
                if (!cadastrarNovo) ...[
                  ...
                  DropdownButtonFormField<Funcionario?>(
                    ...
                    dropdownColor: const Color(0xFF23272A),
                    style: const TextStyle(color: Colors.white),
                    items: dataService.funcionarios.where((f) => f.ativo).map((funcionario) {
                      return DropdownMenuItem(
                        value: funcionario,
                        child: Text(funcionario.nome),
                      );
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (!cadastrarNovo && funcionarioSelecionado == null) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
  ...
  // Adicionar serviço automaticamente quando selecionar um com adicional
  Future<void> _adicionarServicoAutomatico(
    Servico servico,
    ...
    // Verificar se já existe um serviço idêntico na lista
    final servicoJaExiste = _servicosSelecionados.any((item) =>
        item.descricao == servico.nome &&
        item.valor == precoBase &&
        item.valorAdicional == valorAdicional &&
        item.funcionarioId == funcionario?.id);

    if (servicoJaExiste) {
      debugPrint('>>> Serviço "${servico.nome}" já existe na lista, não adicionando duplicado');
      return;
    }
