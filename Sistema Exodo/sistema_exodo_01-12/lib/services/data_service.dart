...
class DataService extends ChangeNotifier {
  ...
  bool _firebaseHabilitado = true; // Flag para habilitar/desabilitar Firebase
  ...
  Future<void> iniciarSincronizacao() async {
    ...
    if (_produtos.isEmpty) {
      print('>>> ⚠ Nenhum produto encontrado. Carregando dados fictícios...');
    ...
    if (_clientes.isEmpty) {
      print('>>> ⚠ Nenhum cliente encontrado. Carregando dados fictícios...');
    ...
    if (_motoristas.isEmpty) {
      print('>>> ⚠ Nenhum motorista encontrado. Carregando dados fictícios...');
  ...
  Future<void> _carregarDadosDoFirebase() async {
    try {
      print('>>> 🔥 Carregando dados do Firebase (PRINCIPAL)...');
      final dados = await _firebaseService.carregarTudoDoFirebase()
          .timeout(
            const Duration(seconds: 8),
            onTimeout: () {
              print('>>> ⚠ Timeout ao carregar do Firebase (8s) - usando fallback');
  ...
  Future<void> _carregarDadosSalvos() async {
    try {
      ...
      final notasMap =
          await _storage.carregarLista(LocalStorageService.keyNotasEntrada);
