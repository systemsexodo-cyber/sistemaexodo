...
$expectedPath = "C:\Users\USER\Downloads\Sistema Exodo\sistema_exodo_01-12"
if (Test-Path $expectedPath) {
    $projectPath = $expectedPath
}

# Encontrar repositório Git
# Mudar para o diretório do projeto antes de executar comandos Git
Set-Location $projectPath
$gitCheck = git rev-parse --show-toplevel 2>$null
if ($gitCheck) {
    $projectPath = $gitCheck
    Set-Location $projectPath
    Write-Detail "Diretório do projeto Git: $projectPath"
} else {
    Write-Error "ERRO: Não é um repositório Git válido em: $projectPath"
    exit 1
}
...
function Fazer-Commit {
    ...
    # SEMPRE garantir que está na branch modo-dev
    $currentBranch = git branch --show-current 2>$null
    if ($currentBranch -ne "modo-dev") {
        Write-Detail "  Branch atual: $currentBranch. Mudando para modo-dev..."
        $checkoutResult = git checkout modo-dev 2>&1
        if ($LASTEXITCODE -ne 0) {
            # Se modo-dev não existe, criar
            Write-Detail "  Criando branch modo-dev..."
            git checkout -b modo-dev 2>&1 | Out-Null
            if ($LASTEXITCODE -ne 0) {
                Write-Error "  Erro ao criar/mudar para modo-dev"
                return $false
            }
        }
    }
...
# SEMPRE trabalha na branch modo-dev, sem criar branches temporários
function Fazer-Push-Limpo {
    ...
    Write-Detail "  [$currentTime] Preparando push limpo para $targetBranch (sempre na branch modo-dev)..."
    
    # SEMPRE garantir que estamos na branch modo-dev
    $currentBranch = git branch --show-current 2>$null
    if ($currentBranch -ne "modo-dev") {
        Write-Detail "  Mudando para branch modo-dev..."
        $checkoutResult = git checkout modo-dev 2>&1
        if ($LASTEXITCODE -ne 0) {
            # Se modo-dev não existe, criar
            Write-Detail "  Criando branch modo-dev..."
    ...
    try {
        ...
        Write-Detail "  Removendo arquivos grandes do staging..."
        $largeFiles = git ls-files | Where-Object {
            $file = $_
            if (Test-Path $file) {
                $size = (Get-Item $file -ErrorAction SilentlyContinue).Length
        ...
        Write-Detail "  Adicionando alterações..."
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Detail "  Nenhum commit necessário ou erro: $commitResult"
        }
        
        Write-Detail "  Fazendo push para $targetBranch..."
        ...
        # Fazer push diretamente da branch modo-dev
        $pushResult = git push origin modo-dev:$targetBranch --no-verify --set-upstream 2>&1
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            $pushLogLine = "$currentTime | PUSH | Branch: $targetBranch (modo-dev)"
            ...
            # Log de erro
            $logFile = Join-Path $logsDir "erros.log"
            $errorLogLine = "$currentTime | PUSH ERROR | Branch: $targetBranch | $pushResult"
        ...
        # Garantir que estamos na branch modo-dev
