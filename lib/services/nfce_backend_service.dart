...
class NFCeBackendService implements NFCeServiceBase {
  ...
  NFCeBackendService({String? baseUrl}) 
      : baseUrl = baseUrl ?? _getDefaultUrl();
  ...
  Future<NFCe> emitir({
    ...
    try {
      ...
      // Debug: Verificar se certificado está sendo enviado
      final empresaData = requestData['empresa'] as Map<String, dynamic>;
      final certificadoBase64 = empresaData['certificado_base64'] as String? ?? '';
      final certificadoConfig = (empresaData['configuracoes'] as Map<String, dynamic>?)?['certificadoDigitalBytes'] as String? ?? '';
      final senha = empresaData['senhaCertificado'] as String? ?? empresaData['senha_certificado'] as String? ?? '';
      
      debugPrint('>>> [NFCeBackend] ========================================');
      debugPrint('>>> [NFCeBackend] DIAGNÓSTICO DE CERTIFICADO:');
      debugPrint('>>> [NFCeBackend] certificado_base64: ${certificadoBase64.isNotEmpty ? "presente (${certificadoBase64.length} chars)" : "AUSENTE"}');
      debugPrint('>>> [NFCeBackend] configuracoes.certificadoDigitalBytes: ${certificadoConfig.isNotEmpty ? "presente (${certificadoConfig.length} chars)" : "AUSENTE"}');
      debugPrint('>>> [NFCeBackend] senhaCertificado: ${senha.isNotEmpty ? "presente (${senha.length} chars)" : "AUSENTE"}');
      debugPrint('>>> [NFCeBackend] ========================================');
      ...
      final response = await http.post(
        ...
        onTimeout: () {
          throw Exception('Timeout ao comunicar com backend Python. Verifique se o servidor está rodando.');
        },
      );
  ...
  Map<String, dynamic> _prepararDadosEmpresa(Empresa empresa) {
    ...
    final dadosEmpresa = {
      ...
      // Enviar certificado em múltiplos formatos para garantir compatibilidade
      'certificado_base64': certificadoBase64,
      'certificadoDigitalUrl': empresa.certificadoDigitalUrl ?? '',
      'configuracoes': configuracoes,  // Prioridade 1 no backend
      'senhaCertificado': empresa.senhaCertificado ?? '',
      'senha_certificado': empresa.senhaCertificado ?? '',  // Formato alternativo
      'csc': empresa.csc ?? '',
      'csc_id_token': empresa.cscIdToken ?? '',
      'serie': empresa.serieNFCe ?? '1',
      'ambienteHomologacao': empresa.ambienteHomologacao ?? true,
      'ambiente_homologacao': empresa.ambienteHomologacao ?? true,  // Formato alternativo
      ...
      'email': empresa.email ?? '',
      'codigo_municipio': empresa.codigoIBGE ?? '',  // Código IBGE do município
    };
    
    // Log final do que será enviado
    debugPrint('>>> [NFCeBackend] ========================================');
    debugPrint('>>> [NFCeBackend] DADOS FINAIS QUE SERÃO ENVIADOS:');
    debugPrint('>>> [NFCeBackend] certificado_base64: ${dadosEmpresa['certificado_base64'].toString().length > 0 ? "presente (${dadosEmpresa['certificado_base64'].toString().length} chars)" : "AUSENTE"}');
    debugPrint('>>> [NFCeBackend] configuracoes.certificadoDigitalBytes: ${(dadosEmpresa['configuracoes'] as Map).containsKey('certificadoDigitalBytes') ? "presente" : "AUSENTE"}');
    debugPrint('>>> [NFCeBackend] senhaCertificado: ${dadosEmpresa['senhaCertificado'].toString().isNotEmpty ? "presente" : "AUSENTE"}');
    debugPrint('>>> [NFCeBackend] ========================================');
