...
def copy_metadata(package_name: str, recursive: bool = False):
    ...
    while todo:
        ...
        try:
            ...
            logger.warning("Distribution not found for package: %s", package_name)
            continue
        ...
        if not hasattr(dist, '_path'):
            logger.warning("Unsupported distribution type for package %s: %s", package_name, type(dist))
            continue
        ...
        if not isinstance(src_path, Path):
            ...
            if src_parent.is_file() and src_parent.name.endswith('.egg'):
                logger.error(
                    "Cannot collect metadata from zipped egg: %s\n"
                    "PyInstaller >= 6.0 does not support zipped eggs. Please reinstall using modern methods:\n"
                    "  1. pip uninstall %s\n"
                    "  2. python -m pip install --upgrade pip setuptools\n"
                    "  3. pip install %s",
                    src_path, package_name, package_name
                )
                continue
            else:
                logger.error(
                    "Unsupported metadata path type for %s: %s (%s)",
                    package_name, src_path, type(src_path)
                )
                continue
        ...
        if src_path.is_dir():
            ...
            logger.warning(
                "Invalid metadata path for %s: %s (not file or directory)",
        ...
        if dist.requires:
            for req in dist.requires:
                try:
                    parsed_req = packaging.requirements.Requirement(req)
                    if parsed_req.marker is None or parsed_req.marker.evaluate():
                        requirements.append(parsed_req.name)
                except packaging.requirements.InvalidRequirement:
                    logger.warning("Invalid requirement in %s: %s", package_name, req)
